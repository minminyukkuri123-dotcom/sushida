// 寿司打 自動入力ボット v1.0
// ブラウザコンソールに貼り付けて実行するスクリプト

(() => {
  'use strict';

  // ワード辞書（日本語ワード → ローマ字入力）
  const WORD_DICT = {
    "ああ言えばこう言う": "aaiebakouiu",
    "開いた口がふさがらない": "aitakutigafusagaranai",
    "愛に国境はない": "ainikokkyouhanai",
    "青い空、白い雲": "aoisora,siroikumo",
    "赤いチューリップ": "akaityu-rippu",
    "赤ちゃんから目が離せない": "akathankaramegahanasenai",
    "アカデミー賞受賞": "akademi-shoujushou",
    "あけましておめでとう": "akemasiteomedetou",
    "あこがれのセーラー服": "akogarenose-ra-fuku",
    "アサリのお味噌汁": "asarinoomisosiru",
    "足がしびれました": "asigasibiremasita",
    "あしたからダイエット": "asitakaradaietto",
    "あした天気になーれ": "asitatenkinina-re",
    "あしたはあしたの風が吹く": "asitahaasitanokazegafuku",
    "あすの天気予報は雨です": "asunotenkiyohou",
    "あせると間違えますよ": "aserutomatigaemasuyo",
    "新しい ゲームソフト": "atarasiige-musofuto",
    "アップルパイを焼きました": "appurupaiwoyakimasita",
    "あとは野となれ山となれ": "atohanotonareyamatonare",
    "アナゴ一本握り": "anagoippoxnnigiri",
    "あなたに一目ぼれ": "anatanihitomebore",
    "あの夕日に向かってダッシュだ": "anoyuuhinimukattedasshuda",
    "アプリケーションエラーです": "apurike-shoxnera-desu",
    "甘いものは別腹": "amaimonohabetubara",
    "甘酸っぱい思い出": "amazuppaiomoide",
    "雨上がりの夜空": "ameagarinoyozora",
    "雨がやんだら虹が出た": "amegayandaranijigadeta",
    "アメリカ連邦捜査局": "amerikarenpousousakyoku",
    "アメリカンショートヘアー": "amerikansho-tohea-",
    "アメリカンフットボール": "amerikanfuttobo-ru",
    "ありがとうございました": "arigatougozaimasita",
    "アリの巣の観察": "arinosunokansatu",
    "アルバイト募集中": "arubaitoboshuutyuu",
    "暗証番号": "anshoubangou",
    "いいニュースと悪いニュース": "iinyu-sutowaruinyu-su",
    "家に帰るまでが遠足": "ienikaerumadegaensoku",
    "いざ尋常に勝負": "izajinjounishoubu",
    "石の上にも三年": "isinouenimosaxnnexn",
    "伊勢海老食べ放題": "iseebitabehoudai",
    "イソップ物語": "isoppumonogatari",
    "痛いの痛いの飛んで行け": "itainoitainotondeike",
    "痛かったら右手を上げてください": "itakattaramigitewoagetekudasai",
    "イチゴショート ケーキ": "itigosho-toke-ki",
    "いつからそこにいたんですか？": "itukarasokoniitandesuka?",
    "一球入魂": "ikkyuunyuukoxn",
    "いつ見てもきれいだね": "itumitemokireidane",
    "犬は英語でドッグです": "inuhaeigodedoggudesu",
    "イヌも歩けば棒にあたる": "inumoarukebabouniataru",
    "犬用シャンプー": "inuyoushanpu-",
    "今何時ですか？": "imananjidesuka?",
    "今まで本当にお世話になりました": "imamadehontouniosewaninarimasita",
    "言われた通りにやったのに": "iwaretatooriniyattanoni",
    "インクジェットプリンター": "inkujettopurinta-",
    "インスピレーション": "insupire-shoxn",
    "インターナショナル": "inta-nashonaru",
    "インターネットの仕組み": "inta-nettonosikumi",
    "インテリアコーディネーター": "interiako-dhine-ta-",
    "ウイスキーをオンザロックで": "uisuki-woonzarokkude",
    "上から読んでも新聞紙": "uekarayondemosinbunsi",
    "ウクレレ教室": "ukurerekyousitu",
    "ウサギをモフモフする": "usagiwomofumofusuru",
    "嘘ついたら針千本飲ーます": "usotuitaraharisenboxnno-masu",
    "嘘つきは泥棒の始まり": "usotukihadorobounohajimari",
    "宇宙人に会いましたよ": "utyuujixnniaimasitayo",
    "宇宙船地球号": "utyuusentikyuugou",
    "美しい日本語": "utukusiinihongo",
    "馬の耳に念仏": "umanomimininenbutu",
    "生まれ変わった僕を見てください": "umarekawattabokuwomitekudasai",
    "運命なんて信じない": "unmeinantesinjinai",
    "運命の赤い糸": "unmeinoakaiito",
    "運も実力のうち": "unmojituryokunouti",
    "英会話教室": "eikaiwakyousitu",
    "英会話レッスン": "eikaiwaressuxn",
    "映画館で昼寝": "eigakandehirune",
    "エイリアンがやってきた": "eiriangayattekita",
    "液晶ディスプレイ": "ekishoudhisupurei",
    "エサを与えないで下さい": "eswoataenaidekudasai",
    "枝毛が増えて気になります": "edagegafuetekininarimasu",
    "エネルギー節約": "enerugi-setuyaku",
    "エビフライ定食": "ebifuraiteishoku",
    "エルニーニョ現象": "eruni-nyogenshou",
    "遠距離恋愛": "enkyorirexnai",
    "エンゼルフィッシュ": "enzerufisshu",
    "縁の下の力持ち": "exnnositanotikaramoti",
    "遠慮しないでたくさん食べてね": "enryosinaidetakusantabetene",
    "お遊びはここまでだ": "oasobihakokomadeda",
    "おいしいお肉が食べたいな": "oisiionikugatabetaina",
    "大食い選手権": "ooguisenshukexn",
    "大化けの可能性あり": "oobakenokanouseiari",
    "おかげ様で元気です": "okagesamadegenkidesu",
    "お気に入りに登録": "okiniirinitouroku",
    "お酒は大人になってから": "osakehaotonaninattekara",
    "おじいさんは山へ柴刈りに": "ojiisanhayamahesibakarini",
    "おしゃれなデザイン": "osharenadezaixn",
    "お醤油取って下さい": "oshouyutottekudasai",
    "お中元の季節": "otyuugexnnokisetu",
    "お疲れ様でした": "otukaresamadesita",
    "お手洗いはあちらです": "otearaihaatiradesu",
    "男ばっかり三兄弟": "otokobakkarisankyoudai",
    "お年玉ちょうだい": "otosidamatyoudai",
    "鬼の居ぬ間に洗濯": "oninoinumanisentaku",
    "鬼は外、福は内": "onihasoto,fukuhauti",
    "おばあさんは川へ洗濯に": "obaasanhakawahesentakuni",
    "主な登場人物": "omonatoujoujinbutu",
    "おやつは戸棚に入っています": "oyatuhatodananihaitteimasu",
    "お料理教室": "oryourikyousitu",
    "温泉に行きたいな": "onsexnniikitaina",
    "女心と秋の空": "oxnnagokorotoakinosora",
    "オンラインショッピング": "onrainshoppingu",
    "恩を仇で返す": "onwoadadekaesu",
    "ガーデニングが趣味です": "ga-deningugashumidesu",
    "飼い犬に手を咬まれる": "kaiinunitewokamareru",
    "海水浴でスイカ割り": "kaisuiyokudesuikawari",
    "懐中電灯": "kaityuudentou",
    "解答欄に記入してください": "kaitouraxnnikinyuusitekudasai",
    "海洋深層水": "kaiyousinsousui",
    "カエルの子はカエル": "kaerunokohakaeru",
    "カキ氷始めました": "kakigoorihajimemasita",
    "隠し味はお醤油": "kakusiajihaoshouyu",
    "隠れても無駄だ！": "kakuretemomudada!",
    "駆け込み乗車はおやめください": "kakekomijoushahaoyamekudasai",
    "傘でゴルフの練習": "kasadegorufunorenshuu",
    "貸切露天風呂": "kasikirirotenburo",
    "火星人襲来": "kaseijinshuurai",
    "学校給食": "gakkoukyuushoku",
    "学校の七不思議": "gakkounonanafusigi",
    "果肉入りオレンジジュース": "kanikuiriorenjiju-su",
    "カブトムシ対クワガタムシ": "kabutomusitaikuwagatamusi",
    "壁に耳あり障子に目あり": "kabenimimiarishoujinimeari",
    "亀の甲より年の功": "kamenokouyoritosinokou",
    "火曜日は定休日です": "kayoubihateikyuubidesu",
    "カラクリ人形": "karakuriningyou",
    "辛子明太子おにぎり": "karasimentaikoonigiri",
    "カリフォルニアロール": "kariforuniaro-ru",
    "カルシウムが足りない": "karusiumugatarinai",
    "カレーライスとハンバーグが好きです": "kare-raisutohanba-gugasukidesu",
    "可愛いチワワですね": "kawaiitiwawadesune",
    "変わった趣味をお持ちですね": "kawattashumiwoomotidesune",
    "関西国際空港": "kansaikokusaikuukou",
    "完全燃焼": "kanzexnnenshou",
    "感動的な再会": "kandoutekinasaikai",
    "カンヌ国際映画祭": "kaxnnukokusaieigasai",
    "頑張ってください": "ganbattekudasai",
    "観葉植物": "kaxnyoushokubutu",
    "記憶にございません": "kiokunigozaimasexn",
    "期待して損した": "kitaisitesonsita",
    "キツネとタヌキの化かし合い": "kitunetotanukinobakasiai",
    "昨日のことは覚えていません": "kinounokotohaoboeteimasexn",
    "昨日の敵は今日の友": "kinounotekihakyounotomo",
    "逆転サヨナラ満塁ホームラン": "gyakutensayonaramanruiho-muraxn",
    "キャビアはチョウザメの卵": "kyabiahatyouzamenotamago",
    "キャラメルポップコーン": "kyaramerupoppuko-xn",
    "キャンプファイヤー": "kyanpufaiya-",
    "キャンペーン実施中": "kyanpe-njissityuu",
    "究極の選択": "kyuukyokunosentaku",
    "給食当番": "kyuushokutoubaxn",
    "牛肉のカルパッチョ": "gyuunikunokarupattyo",
    "驚愕の事実": "kyougakunojijitu",
    "強制終了": "kyouseishuuryou",
    "今日のおすすめ品": "kyounoosusumehixn",
    "今日はいい天気ですね": "kyouhaiitenkidesune",
    "今日は楽しかったよ": "kyouhatanosikattayo",
    "行列の絶えない人気店": "gyouretunotaenaininkitexn",
    "きれいな花にはトゲがある": "kireinahananihatogegaaru",
    "キンキンに冷えたビール": "kinkixnnihietabi-ru",
    "禁断の魔術": "kindaxnnomajutu",
    "筋肉は裏切らない": "kixnnikuhauragiranai",
    "金の斧ですか銀の斧ですか": "kixnnoonodesukagixnnoonodesuka",
    "グアムとサイパンは近い": "guamutosaipanhatikai",
    "空気清浄機": "kuukiseijouki",
    "空中ブランコ": "kuutyuuburanko",
    "雲行きが怪しい": "kumoyukigaayasii",
    "クラシック音楽": "kurasikkuongaku",
    "グラフィックデザイナー": "gurafikkudezaina-",
    "クリスマスツリー点灯": "kurisumasuturi-tentou",
    "クリスマスパーティー": "kurisumasupa-thi-",
    "くるみ割り人形": "kurumiwariningyou",
    "経営コンサルタント": "keieikonsarutanto",
    "継続は力なり": "keizokuhatikaranari",
    "携帯の電源をお切り下さい": "keitainodengenwookirikudasai",
    "血液型占い": "ketuekigatauranai",
    "結婚おめでとう": "kekkoxnomedetou",
    "決定的瞬間": "ketteitekishunkaxn",
    "ゲルマン民族の大移動": "gerumanminzokunodaiidou",
    "後悔先に立たず": "koukaisakinitatazu",
    "合格発表": "goukakuhappyou",
    "高級チョコレート": "koukyuutyokore-to",
    "甲子園球場": "kousienkyuujou",
    "紅 茶専門店": "koutyasenmontexn",
    "交通ルールを守りましょう": "koutuuru-ruwomamorimashou",
    "コウモリは哺乳類です": "koumorihahonyuuruidesu",
    "コーヒー牛乳": "ko-hi-gyuunyuu",
    "コーヒーにミルク入れる？": "ko-hi-nimirukuireru?",
    "ご活躍をお祈りします": "gokatuyakuwooinorisimasu",
    "極秘プロジェクト始動": "gokuhipurojekutosidou",
    "国民健康保険": "kokuminkenkouhokexn",
    "ここからが本番です": "kokokaragahonbandesu",
    "ここだけの話ですが": "kokodakenohanasidesuga",
    "ここで会ったが百年目": "kokodeattagahyakunenme",
    "ここは俺が食い止める！": "kokohaoregakuitomeru!",
    "ここはどこ、私は誰": "kokohadoko,watasihadare",
    "ご自由にお持ち下さい": "gojiyuuniomotikudasai",
    "こたつから出られない": "kotatukaraderarenai",
    "こたつでみかんを食べる": "kotatudemikanwotaberu",
    "ごちそうさまでした": "gotisousamadesita",
    "言葉の魔術師": "kotobanomajutusi",
    "子供の頃からの夢でした": "kodomonokorokaranoyumedesita",
    "子猫の肉球": "konekononikukyuu",
    "コラーゲン配合": "kora-genhaigou",
    "ご利用は計画的に": "goriyouhakeikakutekini",
    "これは訓練ではない": "korehakunrendehanai",
    "これはペンですか？": "korehapendesuka?",
    "怖い話しようぜ": "kowaihanasisiyouze",
    "今週のベストセラー": "konshuunobesutosera-",
    "今度は大丈夫です": "kondohadaijoubudesu",
    "コンビニエンスストア": "konbiniensusutoa",
    "今夜はカレーライスよ": "koxnyahakare-raisuyo",
    "今夜はロールキャベツです": "koxnyaharo-rukyabetudesu",
    "サービス残業": "sa-bisuzangyou",
    "再起動が必要です": "saikidougahituyoudesu",
    "最高級食材": "saikoukyuushokuzai",
    "最高級のおもてなし": "saikoukyuunoomotenasi",
    "最高新記録": "saikousinkiroku",
    "最高の雪景色": "saikounoyukigesiki",
    "再就職先": "saishuushokusaki",
    "財布が小銭でパンパンです": "saifugakozenidepanpandesu",
    "桜前線北上中": "sakurazensenhokujoutyuu",
    "鮭とイクラの親子丼": "saketoikuranooyakodoxn",
    "酒は飲んでも飲まれるな": "sakehanondemonomareruna",
    "砂糖と塩を間違えた": "satoutosioomatigaeta",
    "寒い日が続いています": "samuihigatuduiteimasu",
    "三角関数の応用": "sankakukansuunoouyou",
    "産業革命": "sangyoukakumei",
    "残念なお知らせがあります": "zaxnnexnnaosirasegaarimasu",
    "サンマが安いですよ": "sanmagayasuidesuyo",
    "ジェットコースター": "jettoko-suta-",
    "シェフの気まぐれサラダ": "shefunokimaguresarada",
    "四角い畳を丸く掃く": "sikakuitatamiwomarukuhaku",
    "自己紹介をしてください": "jikoshoukaiwositekudasai",
    "獅子座流星群": "sisizaryuuseiguxn",
    "下から読んでも新聞紙": "sitakarayondemosinbunsi",
    "ジブラルタル海峡": "jiburarutarukaikyou",
    "自分に対するご褒美": "jibuxnnitaisurugohoubi",
    "しまった、寝過ごした！": "simatta,nesugosita!",
    "社会の歯車": "shakainohaguruma",
    "弱点が丸見え": "jakutengamarumie",
    "弱肉強食": "jakunikukyoushoku",
    "シャッターチャンス": "shatta-tyansu",
    "修学旅行": "shuugakuryokou",
    "集合写真": "shuugoushasin",
    "集中豪雨": "shuutyuugouu",
    "シューティング ゲーム": "shu-thinguge-mu",
    "宿題を忘れました": "shukudaiwowasuremasita",
    "受験勉強": "jukenbenkyou",
    "出血大サービス": "shukketudaisa-bisu",
    "趣味は お茶 茶とお花とお琴です": "shumihaotyatoohanatookotodesu",
    "需要と供給": "juyoutokyoukyuu",
    "障害物競走": "shougaibutukyousou",
    "場外ホームラン": "jougaiho-muraxn",
    "上昇気流": "joushoukiryuu",
    "使用上の注意": "siyoujounotyuui",
    "少年よ大志を抱け": "shounexnyotaisiwoidake",
    "商売繁盛": "shoubaihanjou",
    "少林寺拳法": "shourinjikenpou",
    "食後のコーヒー": "shokugonoko-hi-",
    "食虫植物": "shokutyuushokubutu",
    "ショッピングセンター": "shoppingusenta-",
    "処理速度を高速化": "shorisokudowokousokuka",
    "書類にハンコを押してください": "shoruinihankowoositekudasai",
    "シルバーアクセサリー": "siruba-akusesari-",
    "白いご飯が食べたい": "siroigohangatabetai",
    "真珠のネックレス": "sinjunonekkuresu",
    "人生の分岐点": "jinseinobunkitexn",
    "人生山あり谷あり": "jinseiyamaaritaniari",
    "心臓に毛が生えている": "sinzounikegahaeteiru",
    "新築マンション": "sintikumanshoxn",
    "シンデレラストーリー": "sindererasuto-ri-",
    "新東京国際空港": "sintoukyoukokusaikuukou",
    "新入社員": "sixnnyuushaixn",
    "新入生歓迎会": "sixnnyuuseikangeikai",
    "心理学を専攻する": "sinrigakuwosenkousuru",
    "スイーツ食べ放題": "sui-tutabehoudai",
    "水酸化ナトリウム水溶液": "suisankanatoriumusuiyoueki",
    "ずいずいずっころばし": "zuizuizukkorobasi",
    "睡魔には勝てなかった": "suimanihakatenakatta",
    "素顔は見せられない": "sugaohamiserarenai",
    "隙間に入っちゃった": "sukimanihaittyatta",
    "スキューバダイビング": "sukyu-badaibingu",
    "スクランブル交差点": "sukuranburukousatexn",
    "ずっと遊んで暮らしたい": "zuttoasondekurasitai",
    "ずっとこのまま寝ていたい": "zuttokonomamaneteitai",
    "ストレス解消": "sutoresukaishou",
    "スペースキーを連打": "supe-suki-worenda",
    "スペシャルサンクス": "supesharusankusu",
    "スペシャルプライス": "supesharupuraisu",
    "スモモも桃も桃のうち": "sumomomomomomomomonouti",
    "生死の境をさまよう": "seisinosakaiwosamayou",
    "世界一周旅行": "sekaiisshuuryokou",
    "石油ファンヒーター": "sekiyufanhi-ta-",
    "赤裸々な告白": "sekiraranakokuhaku",
    "絶体絶命の大ピンチ": "zettaizetumeinodaipinti",
    "選挙管理委員会": "senkyokanriiinkai",
    "先行試写会": "senkousishakai",
    "戦国大名": "sengokudaimyou",
    "全財産をつぎ込む": "zenzaisanwotugikomu",
    "全自動皿洗い機": "zenjidousaraaraiki",
    "全自動洗濯機": "zenjidousentakuki",
    "先進国首脳会議": "sensinkokushunoukaigi",
    "先生の口癖": "senseinokutiguse",
    "戦闘を開始してください": "sentouwokaisisitekudasai",
    "全力を尽くします": "zenryokuwotukusimasu",
    "相対性理論": "soutaiseiriroxn",
    "早朝ランニング": "soutyouraxnningu",
    "そこを右折してください": "sokowousetusitekudasai",
    "そこをまっすぐ行ってください": "sokowomassuguittekudasai",
    "卒業写真": "sotugyoushasixn",
    "卒業文集": "sotugyoubunshuu",
    "そっくりそのまま返す": "sokkurisonomamakaesu",
    "そっちは行き止まりだ！": "sottihaikidomarida!",
    "それでも地球は回っている": "soredemotikyuuhamawatteiru",
    "そろそろ心が折れそうです": "sorosorokokorogaoresoudesu",
    "そんな恐い顔するなって": "soxnnakowaikaosurunatte",
    "体育館に集合": "taiikukaxnnishuugou",
    "大切な書類": "taisetunashorui",
    "代表取締役": "daihyoutorisimariyaku",
    "台風接近中": "taifuusekkintyuu",
    "タイムパラドックス": "taimuparadokkusu",
    "太陽系第三惑星地球": "taiyoukeidaisanwakuseitikyuu",
    "体力測定": "tairyokusokutei",
    "ダウンロードを開始します": "daunro-dowokaisisimasu",
    "ただ今準備中です": "tadaimajunbityuudesu",
    "畳にキノコが生えました": "tataminikinokogahaemasita",
    "タダより高いものは無い": "tadayoritakaimonohanai",
    "卓球で脱臼": "takkyuudedakkyuu",
    "脱出成功！": "dasshutuseikou!",
    "立つ鳥跡を濁さず": "tatutoriatowonigosazu",
    "タバコがやめられない": "tabakogayamerarenai",
    "タピオカミルクティー": "tapiokamirukuthi-",
    "旅に出たい気分": "tabinidetaikibuxn",
    "食べたら歯を磨こうね": "tabetarahawomigakoune",
    "食べてすぐ寝たら牛になりました": "tabetesugunetarausininarimasita",
    "魂の一撃": "tamasiinoitigeki",
    "だるまさんが転んだ": "darumasangakoronda",
    "誰も反応してくれない": "daremohaxnnousitekurenai",
    "炭酸飲料": "tansaxninryou",
    "誕生日おめでとう": "tanjoubiomedetou",
    "誕生日プレゼント": "tanjoubipurezento",
    "タンスの角に小指をぶつけた": "tansunokadonikoyubiwobutuketa",
    "地球外生命体": "tikyuugaiseimeitai",
    "地球は青かった": "tikyuuhaaokatta",
    "乳搾り体験": "titisiboritaikexn",
    "チャレンジ精神": "tyarenjiseisixn",
    "チューリップが咲きました": "tyu-rippugasakimasita",
    "超高級リゾートホテル": "tyoukoukyuurizo-tohoteru",
    "超常現象": "tyoujougenshou",
    "調理師免許": "tyourisimenkyo",
    "チョコレートパフェ": "tyokore-topafe",
    "ちょっと休憩": "tyottokyuukei",
    "ちょっと待ってください": "tyottomattekudasai",
    "賃貸マンション": "tintaimanshoxn",
    "使い捨てコンタクトレンズ": "tukaisutekontakutorenzu",
    "月がきれいですね": "tukigakireidesune",
    "低気圧が発達中": "teikiatugahattatutyuu",
    "ディスカウントショップ": "dhisukauntoshoppu",
    "出口のない迷路": "degutinonaimeiro",
    "テスト期間まであと一週間": "tesutokikanmadeatoisshuukaxn",
    "電気を大切に": "denkiwotaisetuni",
    "天井裏になにかいる": "tenjouuraninanikairu",
    "伝説の勇者": "densetunoyuusha",
    "ドアノブが取れました": "doanobugatoremasita",
    "トイレはどこですか？": "toirehadokodesuka?",
    "東京と京都": "toukyoutokyouto",
    "どうしても思い出せない": "dousitemoomoidasenai",
    "東北新幹線": "touhokusinkansexn",
    "特賞はハワイ旅行": "tokushouhahawairyokou",
    "読書感想文": "dokushokansoubuxn",
    "匿名希望さん": "tokumeikibousaxn",
    "時計が壊れたんです": "tokeigakowaretandesu",
    "どこかでお会いしませんでしたか？": "dokokadeoaisimasendesitaka?",
    "どこからか視線を感じる": "dokokarakasisenwokanjiru",
    "特許出願中": "tokkyoshutugantyuu",
    "隣の客はよく柿食う客だ": "tonarinokyakuhayokukakikuukyakuda",
    "ドラゴン退治に出発だ": "doragontaijinishuppatuda",
    "取らぬ狸の皮算用": "toranutanukinokawazaxnyou",
    "トラブルシューティング": "toraburushu-thingu",
    "ドラマチックな出会い": "doramatikkunadeai",
    "とりあえずビールちょうだい": "toriaezubi-rutyoudai",
    "ドレミファソラシド": "doremifasorasido",
    "トロピカルフルーツ": "toropikarufuru-tu",
    "長い夜になりそうです": "nagaiyoruninarisoudesu",
    "仲良しこよしのお友達": "nakayosikoyosinootomodati",
    "ナスカの地上絵": "nasukanotijoue",
    "謎の多い人物": "nazonoooijinbutu",
    "夏休みの絵日記": "natuyasuminoenikki",
    "夏休みの宿題終わった？": "natuyasuminoshukudaiowatta?",
    "生むぎ生ごめ生たまご": "namamuginamagomenamatamago",
    "南極大陸": "nankyokutairiku",
    "何だか嫌な予感がする": "nandakaiyanayokangasuru",
    "二階から目薬": "nikaikaramegusuri",
    "日本国憲法": "nihonkokukenpou",
    "日本の首都は東京": "nihoxnnoshutohatoukyou",
    "庭には二羽にわとりがいる": "niwanihaniwaniwatorigairu",
    "人気テーマパーク": "ninkite-mapa-ku",
    "脱いだら脱ぎっぱなし": "nuidaranugippanasi",
    "抜け道を発見": "nukemitiwohakkexn",
    "猫の首に鈴をつける": "nekonokubinisuzuwotukeru",
    "猫の手も借りたいです": "nekonotemokaritaidesu",
    "熱狂的サポーター": "nekkyoutekisapo-ta-",
    "熱帯雨林気候": "nettaiurinkikou",
    "ノーヒットノーラン": "no-hittono-raxn",
    "ノンアルコールビール": "noxnaruko-rubi-ru",
    "パーソナル コンピュータ": "pa-sonarukonpyu-ta",
    "バイオテクノロジー": "baiotekunoroji-",
    "パイナップルの缶詰": "painappurunokandume",
    "爆発的人気": "bakuhatutekininki",
    "白馬に乗った王子様": "hakubaninottaoujisama",
    "激しいリンボーダンス": "hagesiirinbo-dansu",
    "はさみでチョキチョキ": "hasamidetyokityoki",
    "パスワードを入れてください": "pasuwa-dowoiretekudasai",
    "パスワードを入力してください": "pasuwa-dowonyuuryokusitekudasai",
    "働かざるもの食うべからず": "hatarakazarumonokuubekarazu",
    "働くパパはカッコイイ": "hatarakupapahakakkoii",
    "パパ、ぼく弟ほしい": "papa,bokuotoutohosii",
    "バミューダトライアングル": "bamyu-datoraianguru",
    "早起きは三文の得": "hayaokihasanmoxnnotoku",
    "張りつめた空気が漂う": "haritumetakuukigatadayou",
    "はるか遠くの銀河": "harukatookunoginga",
    "パン食い競争": "pankuikyousou",
    "犯人はこの中にいる": "haxnninhakononakaniiru",
    "ハンバーグステーキ": "hanba-gusute-ki",
    "ハンバーグ定食": "hanba-guteishoku",
    "ピーマンも食べなさい": "pi-manmotabenasai",
    "ビールのおいしい季節": "bi-runooisiikisetu",
    "非常識にも程がある": "hijousikinimohodogaaru",
    "非常事態発生": "hijoujitaihassei",
    "引っ越しの準備": "hikkosinojunbi",
    "火に油を注ぐ": "hiniaburawososogu",
    "冷やし中華始めました": "hiyasityuukahajimemasita",
    "ヒラメとカレイの見分け方": "hirametokareinomiwakekata",
    "ファーストクラスに乗ってみたい": "fa-sutokurasuninottemitai",
    "ファーブル昆虫記": "fa-burukontyuuki",
    "ファッション雑誌": "fasshonzassi",
    "ファッションセンスに難あり": "fasshonsensuninaxnari",
    "ファミリーレストラン": "famiri-resutoraxn",
    "風力発電": "fuuryokuhatudexn",
    "プールサイドで日光浴": "pu-rusaidodenikkouyoku",
    "ふかふかのお布団で眠りたい": "fukafukanoofutondenemuritai",
    "普段着はパジャマです": "fudangihapajamadesu",
    "布団が恋しい季節": "futongakoisiikisetu",
    "布団から出られない": "futonkaraderarenai",
    "冬将軍の到来": "fuyushouguxnnotourai",
    "ブラウザは何を使っていますか？": "burauzahananiwotukatteimasuka?",
    "鰤と書いてブリと読む": "buritokaiteburitoyomu",
    "ブリとハマチは同じ魚": "buritohamatihaonajisakana",
    "ブルータス、お前もか": "buru-tasu,omaemoka",
    "フルーツの王様": "furu-tunoousama",
    "フルモデルチェンジ": "furumoderutyenji",
    "プレゼンテーション": "purezente-shoxn",
    "風呂上りの一杯": "furoagarinoippai",
    "プログラミング言語": "puroguramingugengo",
    "プロフェッショナル": "purofesshonaru",
    "ふわふわパン ケーキ": "fuwafuwapanke-ki",
    "平均寿命": "heikinjumyou",
    "ヘッドスライディング": "heddosuraidhingu",
    "豊富なボキャブラリー": "houfunabokyaburari-",
    "ホームページ作りました": "ho-mupe-jitukurimasita",
    "ホコリでくしゃみ連発": "hokoridekushamirenpatu",
    "星のない夜の空": "hosinonaiyorunosora",
    "母性本能をくすぐる": "boseihoxnnouwokusuguru",
    "北海道直送": "hokkaidoutyokusou",
    "北極と南極": "hokkyokutonankyoku",
    "微笑みの貴公子": "hohoeminokikousi",
    "ホワイトチョコレート": "howaitotyokore-to",
    "本当にあった怖い話": "hontouniattakowaihanasi",
    "ほんの出来心だったんです": "hoxnnodekigokorodattandesu",
    "迷子のご案内": "maigonogoaxnnai",
    "枕投げしようぜ": "makuranagesiyouze",
    "マサチューセッツ工科大学": "masatyu-settukoukadaigaku",
    "またのお越しをお待ちしております": "matanookosiwoomatisiteorimasu",
    "全く覚えていない": "mattakuoboeteinai",
    "窓の外は冷たい雨": "madonosotohatumetaiame",
    "マヤ文明の遺跡": "mayabunmeinoiseki",
    "マンションの一室": "manshoxnnoissitu",
    "ミイラ取りがミイラになる": "miiratorigamiiraninaru",
    "未確認飛行物体": "mikakuninhikoubuttai",
    "ミステリーサークル": "misuteri-sa-kuru",
    "味噌バターコーンラーメン": "misobata-ko-nra-mexn",
    "味噌ラーメン大盛り": "misora-mexnoomori",
    "未知の生命体": "mitinoseimeitai",
    "未知の領域に踏み込む": "mitinoryouikinifumikomu",
    "見つかってしまった": "mitukattesimatta",
    "見逃してください！": "minogasitekudasai!",
    "身分証明書": "mibunshoumeisho",
    "魅力的なアイテム": "miryokutekinaaitemu",
    "無人島生活": "mujintouseikatu",
    "無人島でサバイバル": "mujintoudesabaibaru",
    "無人島に漂着": "mujintounihyoutyaku",
    "無遅刻無欠席": "mutikokumukesseki",
    "迷惑メールが多すぎる": "meiwakume-rugaoosugiru",
    "メールアドレスを間違えた": "me-ruadoresuwomatigaeta",
    "目覚まし時計に勝ちました": "mezamasidokeinikatimasita",
    "メジャーリーグへ挑戦": "meja-ri-guhetyousexn",
    "珍しく真剣な顔してるね": "mezurasikusinkexnnakaositerune",
    "メンテナンスフリー": "mentenansufuri-",
    "もう一度お掛け直し下さい": "mouitidookakenaosikudasai",
    "もう一度チャンスをください": "mouitidotyansuwokudasai",
    "もう一度やり直そう": "mouitidoyarinaosou",
    "毛細管現象": "mousaikangenshou",
    "もう少し待ってください": "mousukosimattekudasai",
    "もうだめだ、降参しよう": "moudameda,kousansiyou",
    "もうどうにでもなーれ": "moudounidemona-re",
    "木工用ボンド": "mokkouyoubondo",
    "桃がドンブラコと流れてきました": "momogadonburakotonagaretekimasita",
    "盛り上がってまいりました": "moriagattemairimasita",
    "野球中継": "yakyuutyuukei",
    "やっぱりやめておけばよかった": "yappariyameteokebayokatta",
    "遊園地でデート": "yuuentide-to",
    "有給休暇": "yuukyuukyuuka",
    "有給休暇を使う": "yuukyuukyuukawotukau",
    "有効期限が切れました": "yuukoukigengakiremasita",
    "有酸素運動": "yuusansoundou",
    "優柔不断な人ね": "yuujuufudaxnnahitone",
    "優勝おめでとう！": "yuushouomedetou!",
    "優勝トロフィー": "yuushoutorofi-",
    "ユーラシア大陸": "yu-rasiatairiku",
    "浴衣の綺麗なお姉さん": "yukatanokireinaoneesaxn",
    "揺りかごから墓場まで": "yurikagokarahakabamade",
    "よろしくお願いします": "yorosikuonegaisimasu",
    "来年のカレンダー": "rainexnnokarenda-",
    "リクライニングシート": "rikurainingusi-to",
    "リモートコントローラー": "rimo-tokontoro-ra-",
    "リュウグウノツカイ": "ryuuguunotukai",
    "領収書ください": "ryoushuushokudasai",
    "良薬は口に苦し": "ryouyakuhakutininigasi",
    "料理のさしすせそ": "ryourinosasisuseso",
    "料理は愛情": "ryourihaaijou",
    "緑黄色野菜": "ryokuoushokuyasai",
    "ルールがわかりません": "ru-rugawakarimasexn",
    "ルールを守りましょう": "ru-ruwomamorimashou",
    "留守番電話サービスです": "rusubandenwasa-bisudesu",
    "冷蔵庫のプリン知らない？": "reizoukonopurinsiranai?",
    "冷凍 食品": "reitoushokuhixn",
    "レインボーブリッジ": "reinbo-burijji",
    "廊下に立ってなさい": "roukanitattenasai",
    "廊下を走らない": "roukawohasiranai",
    "労働基準法": "roudoukijunhou",
    "ログインしてください": "roguinsitekudasai",
    "六法全書を丸暗記": "roppouzenshowomaruanki",
    "分かった人は手を挙げて": "wakattahitohatewoagete",
    "和食か洋食": "washokukayoushoku",
    "私英語ワカリマセーン": "watasieigowakarimase-xn",
    "ワニ革のハンドバッグ": "wanigawanohandobaggu",
    "和風ドレッシング": "wafuudoressingu",
    "我思う、故に我あり": "wareomou,yueniwareari",
    "ワレワレハウチュウジンダ": "warewarehautyuujinda"
  };

  // ボットの状態
  let isRunning = false;
  let lastWord = '';
  let typingInterval = null;

  // キーイベントを送信する関数
  function sendKey(char) {
    const keyCode = char.charCodeAt(0);
    const key = char.toLowerCase();
    
    // keydown イベント
    const keydownEvent = new KeyboardEvent('keydown', {
      key: key,
      code: 'Key' + key.toUpperCase(),
      keyCode: keyCode,
      which: keyCode,
      bubbles: true,
      cancelable: true
    });
    document.dispatchEvent(keydownEvent);
    
    // keypress イベント
    const keypressEvent = new KeyboardEvent('keypress', {
      key: key,
      code: 'Key' + key.toUpperCase(),
      keyCode: keyCode,
      which: keyCode,
      charCode: keyCode,
      bubbles: true,
      cancelable: true
    });
    document.dispatchEvent(keypressEvent);
    
    // keyup イベント
    const keyupEvent = new KeyboardEvent('keyup', {
      key: key,
      code: 'Key' + key.toUpperCase(),
      keyCode: keyCode,
      which: keyCode,
      bubbles: true,
      cancelable: true
    });
    document.dispatchEvent(keyupEvent);
  }

  // ローマ字を1文字ずつタイプする
  async function typeRomaji(romaji) {
    for (let i = 0; i < romaji.length; i++) {
      if (!isRunning) break;
      const char = romaji[i];
      sendKey(char);
      // 50〜100msのランダムな遅延
      const delay = 50 + Math.random() * 50;
      await new Promise(resolve => setTimeout(resolve, delay));
    }
  }

  // 画面監視用のMutationObserver（Canvasベースなので使えないが念のため）
  function startMonitoring() {
    console.log('🍣 寿司打ボット起動中...');
    console.log('📚 辞書に登録されているワード数:', Object.keys(WORD_DICT).length);
    
    // 手動入力モード：辞書からワードを検索してタイプ
    window.sushidaBot = {
      // ワードを入力（日本語ワードを指定）
      type: async function(word) {
        if (WORD_DICT[word]) {
          console.log('⌨️ 入力:', word, '→', WORD_DICT[word]);
          await typeRomaji(WORD_DICT[word]);
        } else {
          console.log('❌ 辞書にないワード:', word);
        }
      },
      // ローマ字を直接入力
      typeRaw: async function(romaji) {
        console.log('⌨️ ローマ字入力:', romaji);
        await typeRomaji(romaji);
      },
      // 辞書を検索
      search: function(keyword) {
        const results = Object.entries(WORD_DICT).filter(([k, v]) => 
          k.includes(keyword) || v.includes(keyword)
        );
        console.table(results.map(([k, v]) => ({ ワード: k, ローマ字: v })));
        return results;
      },
      // 停止
      stop: function() {
        isRunning = false;
        console.log('⏹️ ボット停止');
      },
      // 開始
      start: function() {
        isRunning = true;
        console.log('▶️ ボット開始');
      },
      // 辞書
      dict: WORD_DICT
    };

    isRunning = true;
    console.log('✅ ボット準備完了！');
    console.log('');
    console.log('📖 使い方:');
    console.log('  sushidaBot.type("ありがとうございました") - ワードを入力');
    console.log('  sushidaBot.typeRaw("arigatou") - ローマ字を直接入力');
    console.log('  sushidaBot.search("ありがとう") - 辞書を検索');
    console.log('  sushidaBot.stop() - 停止');
    console.log('  sushidaBot.start() - 開始');
  }

  // 起動
  startMonitoring();
})();
